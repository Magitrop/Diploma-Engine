...
public:
	...
	std::vector<EntityID> getObjectsWithComponents()
	{
		...
		for (std::size_t i = 0; i < m_objectsWithComponents.size(); ++i)
		{
			...
			if (select)
			{
				objectsManager.push_back(getObject(i));
			}
		}
...
int main()
{
	...
	std::vector<EntityID> objectsManager;
	for (int i = 0; i < 250; ++i)
	{
		EntityID obj = objManager.createObject();
		if (rand() % 100 > 50) objManager.attachComponent<CameraComponent>(obj);
		objManager.attachComponent<TransformComponent>(obj);
		objectsManager.push_back(obj);
	}

	auto v = objManager.getObjectsWithComponents<CameraComponent, TransformComponent>();
